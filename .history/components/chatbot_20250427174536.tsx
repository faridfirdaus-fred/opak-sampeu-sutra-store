...
import { FaRobot, FaTimes, FaShoppingCart, FaTruck, FaBoxOpen, FaClipboardList } from "react-icons/fa";
...
export default function Chatbot() {
  ...
  const sendMessage = async (message: string, fromUser: boolean = true) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { type: "bot", content: "Maaf, ada masalah dengan sistem. Silakan coba lagi nanti." },
      ]);
    }
  };
      ...
      >
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="text-xs flex items-center">
                ...
                Online
                <span className="ml-1 opacity-75">• Respon rata-rata: 30 detik</span>
              </div>
            </div>
          </div>
        ...
        <div className="flex-grow p-4 overflow-y-auto bg-gray-50">
          {messages.map((msg, idx) => (
            <div
              key={idx}
              className={`my-2 ${msg.type === "user" ? "flex justify-end" : "flex justify-start"}`}
            >
              {msg.type === "bot" && (
        ...
        <div className="px-4 py-2 grid grid-cols-2 gap-2 bg-gray-50">
          {[
            { label: "Cek Harga Produk", icon: <FaShoppingCart />, message: "Cek harga produk" },
            { label: "Info Pengiriman", icon: <FaTruck />, message: "Info pengiriman" },
            { label: "Status Pesanan", icon: <FaBoxOpen />, message: "Status pesanan" },
            { label: "Cara Pemesanan", icon: <FaClipboardList />, message: "Cara pemesanan" },
          ].map((btn, idx) => (
            <button
              key={idx}
        ...
        <form onSubmit={handleSubmit} className="p-3 border-t flex items-center">
          ...
          />
          <button type="submit" className="bg-primary text-white p-2 rounded-r-lg hover:bg-[#FF6600]">
