...
return (
  <div className="relative mx-auto w-full h-full px-0 py-16"> {/* Menghapus px-20 */}
    <Carousel className="w-full relative rounded-xl overflow-hidden shadow-lg bg-red-400">
      <CarouselContent className="w-full">
        {slides.map((url, index) => (
          <CarouselItem
            key={index}
            className="w-full h-[500px] flex justify-center items-center" // Menambahkan tinggi tetap
          >
            <img
              src={url}
              alt={`Slide ${index + 1}`}
              className="w-full h-full object-cover" // Menjaga proporsi gambar
            />
          </CarouselItem>
        ))}
      </CarouselContent>
